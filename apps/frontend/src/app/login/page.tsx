...
export default function LoginPage() {
  return (
    <div className="min-h-screen relative flex flex-col justify-between p-6 bg-[#07130C] overflow-hidden font-sans">
      ...
      {/* TOP LOGO - Enlarged, moved upward and to the left! */}
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center w-full max-w-4xl mx-auto my-8">
         {/* Card Background - Synchronized with Signup Page */}
         <div className="w-full bg-[#0A1C12]/95 backdrop-blur-xl rounded-3xl border border-[#1A4B2E]/60 shadow-[0_0_40px_rgba(34,197,94,0.1)] overflow-hidden flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                <div className="text-center mb-8">
                    <h2 className="text-3xl font-bold text-[#22C55E] mb-2">Sign In</h2>
                    <p className="text-white font-medium text-sm">Access your KuryenTrade account</p>
                </div>

                <form className="space-y-5">
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-white ml-1">Email</label>
                        <input type="email" placeholder="Enter your email" className="w-full bg-[#133A24] border border-[#1A4B2E] focus:border-[#22C55E] focus:ring-1 focus:ring-[#22C55E] rounded-xl px-4 py-3.5 text-sm text-white placeholder:text-[#4A8A63] transition-colors outline-none" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-white ml-1">Password</label>
                        <input type="password" placeholder="Enter your password" className="w-full bg-[#133A24] border border-[#1A4B2E] focus:border-[#22C55E] focus:ring-1 focus:ring-[#22C55E] rounded-xl px-4 py-3.5 text-sm text-white placeholder:text-[#4A8A63] transition-colors outline-none" />
                    </div>

                    {/* CHANGED FROM <button> TO <Link> */}
                    <Link href="/dashboard" className="w-full flex justify-center bg-[#22C55E] hover:bg-[#22C55E]/90 text-[#0A1C12] font-bold py-3.5 rounded-xl text-base transition-colors mt-2">
                        Sign In
                    </Link>
                </form>
                ...
                <p className="text-center text-xs text-white mt-6 font-medium">
                    Don't have an account? <Link href="/signup" className="text-[#22C55E] font-bold hover:underline">Sign Up</Link>
                </p>
            </div>
            ...
            <div className="hidden md:block w-1/2 relative p-3">
                <div className="w-full h-full relative rounded-2xl overflow-hidden shadow-inner border border-[#1A4B2E]">
                    <Image src="/images/foreman-businessman-solar-energy-station.jpg" alt="Solar facility" fill className="object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-b from-[#0A1C12]/70 via-transparent to-[#0A1C12]/90"></div>
                    <div className="absolute inset-0 p-8 flex flex-col justify-between">
                        <div className="mt-4">
                            <h2 className="text-4xl lg:text-5xl font-extrabold text-white mb-4 leading-tight">Welcome<br/>Back!</h2>
                            <p className="text-gray-200 text-sm max-w-[220px] leading-relaxed">Trade renewable energy securely through blockchain technology</p>
                        </div>
                        
                        {/* ECO IMPACT WIDGET */}
                        <div className="flex items-center justify-end mt-auto">
                            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl py-2.5 px-4 flex items-center gap-3 shadow-2xl">
                                <div className="relative">
                                    {/* Soft glowing background behind the leaf */}
                                    <div className="absolute inset-0 bg-[#22C55E] blur-md opacity-40 rounded-full"></div>
                                    <Leaf className="w-6 h-6 text-[#22C55E] relative z-10" />
                                </div>
                                <div>
                                  <p className="text-white/60 text-[9px] uppercase tracking-widest font-bold mb-0.5">Global Impact</p>
                                  <p className="text-white text-xs font-bold">1.2M Tons CO₂ Offset</p>
                                </div>
                            </div>
                        </div>
